#[doc = "Register `DMA_RSWDC` reader"]
pub struct R(crate::R<DMA_RSWDC_SPEC>);
impl core::ops::Deref for R {
    type Target = crate::R<DMA_RSWDC_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl From<crate::R<DMA_RSWDC_SPEC>> for R {
    #[inline(always)]
    fn from(reader: crate::R<DMA_RSWDC_SPEC>) -> Self {
        R(reader)
    }
}
#[doc = "Register `DMA_RSWDC` writer"]
pub struct W(crate::W<DMA_RSWDC_SPEC>);
impl core::ops::Deref for W {
    type Target = crate::W<DMA_RSWDC_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::ops::DerefMut for W {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}
impl From<crate::W<DMA_RSWDC_SPEC>> for W {
    #[inline(always)]
    fn from(writer: crate::W<DMA_RSWDC_SPEC>) -> Self {
        W(writer)
    }
}
#[doc = "Field `WDCFRS` reader - Watchdog counter for receive status (RS)"]
pub type WDCFRS_R = crate::FieldReader<u8, u8>;
#[doc = "Field `WDCFRS` writer - Watchdog counter for receive status (RS)"]
pub type WDCFRS_W<'a> = crate::FieldWriter<'a, u32, DMA_RSWDC_SPEC, u8, u8, 8, 0>;
impl R {
    #[doc = "Bits 0:7 - Watchdog counter for receive status (RS)"]
    #[inline(always)]
    pub fn wdcfrs(&self) -> WDCFRS_R {
        WDCFRS_R::new((self.bits & 0xff) as u8)
    }
}
impl W {
    #[doc = "Bits 0:7 - Watchdog counter for receive status (RS)"]
    #[inline(always)]
    pub fn wdcfrs(&mut self) -> WDCFRS_W {
        WDCFRS_W::new(self)
    }
    #[doc = "Writes raw bits to the register."]
    #[inline(always)]
    pub unsafe fn bits(&mut self, bits: u32) -> &mut Self {
        self.0.bits(bits);
        self
    }
}
#[doc = "Ethernet DMA receive state watchdog counter register\n\nThis register you can [`read`](crate::generic::Reg::read), [`write_with_zero`](crate::generic::Reg::write_with_zero), [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`modify`](crate::generic::Reg::modify). See [API](https://docs.rs/svd2rust/#read--modify--write-api).\n\nFor information about available fields see [dma_rswdc](index.html) module"]
pub struct DMA_RSWDC_SPEC;
impl crate::RegisterSpec for DMA_RSWDC_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [dma_rswdc::R](R) reader structure"]
impl crate::Readable for DMA_RSWDC_SPEC {
    type Reader = R;
}
#[doc = "`write(|w| ..)` method takes [dma_rswdc::W](W) writer structure"]
impl crate::Writable for DMA_RSWDC_SPEC {
    type Writer = W;
}
#[doc = "`reset()` method sets DMA_RSWDC to value 0"]
impl crate::Resettable for DMA_RSWDC_SPEC {
    #[inline(always)]
    fn reset_value() -> Self::Ux {
        0
    }
}
