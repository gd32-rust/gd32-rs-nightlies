#[doc = "Register `DAC1_R12DH` reader"]
pub struct R(crate::R<DAC1_R12DH_SPEC>);
impl core::ops::Deref for R {
    type Target = crate::R<DAC1_R12DH_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl From<crate::R<DAC1_R12DH_SPEC>> for R {
    #[inline(always)]
    fn from(reader: crate::R<DAC1_R12DH_SPEC>) -> Self {
        R(reader)
    }
}
#[doc = "Register `DAC1_R12DH` writer"]
pub struct W(crate::W<DAC1_R12DH_SPEC>);
impl core::ops::Deref for W {
    type Target = crate::W<DAC1_R12DH_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::ops::DerefMut for W {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}
impl From<crate::W<DAC1_R12DH_SPEC>> for W {
    #[inline(always)]
    fn from(writer: crate::W<DAC1_R12DH_SPEC>) -> Self {
        W(writer)
    }
}
#[doc = "Field `DAC1_DH` reader - DAC1 12-bit right-aligned data"]
pub type DAC1_DH_R = crate::FieldReader<u16, u16>;
#[doc = "Field `DAC1_DH` writer - DAC1 12-bit right-aligned data"]
pub type DAC1_DH_W<'a> = crate::FieldWriterSafe<'a, u32, DAC1_R12DH_SPEC, u16, u16, 12, 0>;
impl R {
    #[doc = "Bits 0:11 - DAC1 12-bit right-aligned data"]
    #[inline(always)]
    pub fn dac1_dh(&self) -> DAC1_DH_R {
        DAC1_DH_R::new((self.bits & 0x0fff) as u16)
    }
}
impl W {
    #[doc = "Bits 0:11 - DAC1 12-bit right-aligned data"]
    #[inline(always)]
    pub fn dac1_dh(&mut self) -> DAC1_DH_W {
        DAC1_DH_W::new(self)
    }
    #[doc = "Writes raw bits to the register."]
    #[inline(always)]
    pub unsafe fn bits(&mut self, bits: u32) -> &mut Self {
        self.0.bits(bits);
        self
    }
}
#[doc = "DAC1 12-bit right-aligned data holding register\n\nThis register you can [`read`](crate::generic::Reg::read), [`write_with_zero`](crate::generic::Reg::write_with_zero), [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`modify`](crate::generic::Reg::modify). See [API](https://docs.rs/svd2rust/#read--modify--write-api).\n\nFor information about available fields see [dac1_r12dh](index.html) module"]
pub struct DAC1_R12DH_SPEC;
impl crate::RegisterSpec for DAC1_R12DH_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [dac1_r12dh::R](R) reader structure"]
impl crate::Readable for DAC1_R12DH_SPEC {
    type Reader = R;
}
#[doc = "`write(|w| ..)` method takes [dac1_r12dh::W](W) writer structure"]
impl crate::Writable for DAC1_R12DH_SPEC {
    type Writer = W;
}
#[doc = "`reset()` method sets DAC1_R12DH to value 0"]
impl crate::Resettable for DAC1_R12DH_SPEC {
    #[inline(always)]
    fn reset_value() -> Self::Ux {
        0
    }
}
