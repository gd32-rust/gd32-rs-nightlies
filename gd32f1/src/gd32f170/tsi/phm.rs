#[doc = "Register `PHM` reader"]
pub type R = crate::R<PhmSpec>;
#[doc = "Register `PHM` writer"]
pub type W = crate::W<PhmSpec>;
#[doc = "Field `G0P0` reader - G0P0 Schmitt trigger hysteresis mode"]
pub type G0p0R = crate::BitReader;
#[doc = "Field `G0P0` writer - G0P0 Schmitt trigger hysteresis mode"]
pub type G0p0W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G0P1` reader - G0P1 Schmitt trigger hysteresis mode"]
pub type G0p1R = crate::BitReader;
#[doc = "Field `G0P1` writer - G0P1 Schmitt trigger hysteresis mode"]
pub type G0p1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G0P2` reader - G0P2 Schmitt trigger hysteresis mode"]
pub type G0p2R = crate::BitReader;
#[doc = "Field `G0P2` writer - G0P2 Schmitt trigger hysteresis mode"]
pub type G0p2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G0P3` reader - G0P3 Schmitt trigger hysteresis mode"]
pub type G0p3R = crate::BitReader;
#[doc = "Field `G0P3` writer - G0P3 Schmitt trigger hysteresis mode"]
pub type G0p3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G1P0` reader - G1P0 Schmitt trigger hysteresis mode"]
pub type G1p0R = crate::BitReader;
#[doc = "Field `G1P0` writer - G1P0 Schmitt trigger hysteresis mode"]
pub type G1p0W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G1P1` reader - G1P1 Schmitt trigger hysteresis mode"]
pub type G1p1R = crate::BitReader;
#[doc = "Field `G1P1` writer - G1P1 Schmitt trigger hysteresis mode"]
pub type G1p1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G1P2` reader - G1P2 Schmitt trigger hysteresis mode"]
pub type G1p2R = crate::BitReader;
#[doc = "Field `G1P2` writer - G1P2 Schmitt trigger hysteresis mode"]
pub type G1p2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G1P3` reader - G1P3 Schmitt trigger hysteresis mode"]
pub type G1p3R = crate::BitReader;
#[doc = "Field `G1P3` writer - G1P3 Schmitt trigger hysteresis mode"]
pub type G1p3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G2P0` reader - G2P0 Schmitt trigger hysteresis mode"]
pub type G2p0R = crate::BitReader;
#[doc = "Field `G2P0` writer - G2P0 Schmitt trigger hysteresis mode"]
pub type G2p0W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G2P1` reader - G2P1 Schmitt trigger hysteresis mode"]
pub type G2p1R = crate::BitReader;
#[doc = "Field `G2P1` writer - G2P1 Schmitt trigger hysteresis mode"]
pub type G2p1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G2P2` reader - G2P2 Schmitt trigger hysteresis mode"]
pub type G2p2R = crate::BitReader;
#[doc = "Field `G2P2` writer - G2P2 Schmitt trigger hysteresis mode"]
pub type G2p2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G2P3` reader - G2P3 Schmitt trigger hysteresis mode"]
pub type G2p3R = crate::BitReader;
#[doc = "Field `G2P3` writer - G2P3 Schmitt trigger hysteresis mode"]
pub type G2p3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G3P0` reader - G3P0 Schmitt trigger hysteresis mode"]
pub type G3p0R = crate::BitReader;
#[doc = "Field `G3P0` writer - G3P0 Schmitt trigger hysteresis mode"]
pub type G3p0W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G3P1` reader - G3P1 Schmitt trigger hysteresis mode"]
pub type G3p1R = crate::BitReader;
#[doc = "Field `G3P1` writer - G3P1 Schmitt trigger hysteresis mode"]
pub type G3p1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G3P2` reader - G3P2 Schmitt trigger hysteresis mode"]
pub type G3p2R = crate::BitReader;
#[doc = "Field `G3P2` writer - G3P2 Schmitt trigger hysteresis mode"]
pub type G3p2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G3P3` reader - G3P3 Schmitt trigger hysteresis mode"]
pub type G3p3R = crate::BitReader;
#[doc = "Field `G3P3` writer - G3P3 Schmitt trigger hysteresis mode"]
pub type G3p3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G4P0` reader - G4P0 Schmitt trigger hysteresis mode"]
pub type G4p0R = crate::BitReader;
#[doc = "Field `G4P0` writer - G4P0 Schmitt trigger hysteresis mode"]
pub type G4p0W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G4P1` reader - G4P1 Schmitt trigger hysteresis mode"]
pub type G4p1R = crate::BitReader;
#[doc = "Field `G4P1` writer - G4P1 Schmitt trigger hysteresis mode"]
pub type G4p1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G4P2` reader - G4P2 Schmitt trigger hysteresis mode"]
pub type G4p2R = crate::BitReader;
#[doc = "Field `G4P2` writer - G4P2 Schmitt trigger hysteresis mode"]
pub type G4p2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G4P3` reader - G4P3 Schmitt trigger hysteresis mode"]
pub type G4p3R = crate::BitReader;
#[doc = "Field `G4P3` writer - G4P3 Schmitt trigger hysteresis mode"]
pub type G4p3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G5P0` reader - G5P0 Schmitt trigger hysteresis mode"]
pub type G5p0R = crate::BitReader;
#[doc = "Field `G5P0` writer - G5P0 Schmitt trigger hysteresis mode"]
pub type G5p0W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G5P1` reader - G5P1 Schmitt trigger hysteresis mode"]
pub type G5p1R = crate::BitReader;
#[doc = "Field `G5P1` writer - G5P1 Schmitt trigger hysteresis mode"]
pub type G5p1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G5P2` reader - G5P2 Schmitt trigger hysteresis mode"]
pub type G5p2R = crate::BitReader;
#[doc = "Field `G5P2` writer - G5P2 Schmitt trigger hysteresis mode"]
pub type G5p2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `G5P3` reader - G5P3 Schmitt trigger hysteresis mode"]
pub type G5p3R = crate::BitReader;
#[doc = "Field `G5P3` writer - G5P3 Schmitt trigger hysteresis mode"]
pub type G5p3W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    #[doc = "Bit 0 - G0P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g0p0(&self) -> G0p0R {
        G0p0R::new((self.bits & 1) != 0)
    }
    #[doc = "Bit 1 - G0P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g0p1(&self) -> G0p1R {
        G0p1R::new(((self.bits >> 1) & 1) != 0)
    }
    #[doc = "Bit 2 - G0P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g0p2(&self) -> G0p2R {
        G0p2R::new(((self.bits >> 2) & 1) != 0)
    }
    #[doc = "Bit 3 - G0P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g0p3(&self) -> G0p3R {
        G0p3R::new(((self.bits >> 3) & 1) != 0)
    }
    #[doc = "Bit 4 - G1P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g1p0(&self) -> G1p0R {
        G1p0R::new(((self.bits >> 4) & 1) != 0)
    }
    #[doc = "Bit 5 - G1P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g1p1(&self) -> G1p1R {
        G1p1R::new(((self.bits >> 5) & 1) != 0)
    }
    #[doc = "Bit 6 - G1P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g1p2(&self) -> G1p2R {
        G1p2R::new(((self.bits >> 6) & 1) != 0)
    }
    #[doc = "Bit 7 - G1P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g1p3(&self) -> G1p3R {
        G1p3R::new(((self.bits >> 7) & 1) != 0)
    }
    #[doc = "Bit 8 - G2P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g2p0(&self) -> G2p0R {
        G2p0R::new(((self.bits >> 8) & 1) != 0)
    }
    #[doc = "Bit 9 - G2P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g2p1(&self) -> G2p1R {
        G2p1R::new(((self.bits >> 9) & 1) != 0)
    }
    #[doc = "Bit 10 - G2P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g2p2(&self) -> G2p2R {
        G2p2R::new(((self.bits >> 10) & 1) != 0)
    }
    #[doc = "Bit 11 - G2P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g2p3(&self) -> G2p3R {
        G2p3R::new(((self.bits >> 11) & 1) != 0)
    }
    #[doc = "Bit 12 - G3P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g3p0(&self) -> G3p0R {
        G3p0R::new(((self.bits >> 12) & 1) != 0)
    }
    #[doc = "Bit 13 - G3P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g3p1(&self) -> G3p1R {
        G3p1R::new(((self.bits >> 13) & 1) != 0)
    }
    #[doc = "Bit 14 - G3P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g3p2(&self) -> G3p2R {
        G3p2R::new(((self.bits >> 14) & 1) != 0)
    }
    #[doc = "Bit 15 - G3P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g3p3(&self) -> G3p3R {
        G3p3R::new(((self.bits >> 15) & 1) != 0)
    }
    #[doc = "Bit 16 - G4P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g4p0(&self) -> G4p0R {
        G4p0R::new(((self.bits >> 16) & 1) != 0)
    }
    #[doc = "Bit 17 - G4P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g4p1(&self) -> G4p1R {
        G4p1R::new(((self.bits >> 17) & 1) != 0)
    }
    #[doc = "Bit 18 - G4P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g4p2(&self) -> G4p2R {
        G4p2R::new(((self.bits >> 18) & 1) != 0)
    }
    #[doc = "Bit 19 - G4P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g4p3(&self) -> G4p3R {
        G4p3R::new(((self.bits >> 19) & 1) != 0)
    }
    #[doc = "Bit 20 - G5P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g5p0(&self) -> G5p0R {
        G5p0R::new(((self.bits >> 20) & 1) != 0)
    }
    #[doc = "Bit 21 - G5P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g5p1(&self) -> G5p1R {
        G5p1R::new(((self.bits >> 21) & 1) != 0)
    }
    #[doc = "Bit 22 - G5P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g5p2(&self) -> G5p2R {
        G5p2R::new(((self.bits >> 22) & 1) != 0)
    }
    #[doc = "Bit 23 - G5P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    pub fn g5p3(&self) -> G5p3R {
        G5p3R::new(((self.bits >> 23) & 1) != 0)
    }
}
impl W {
    #[doc = "Bit 0 - G0P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g0p0(&mut self) -> G0p0W<PhmSpec> {
        G0p0W::new(self, 0)
    }
    #[doc = "Bit 1 - G0P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g0p1(&mut self) -> G0p1W<PhmSpec> {
        G0p1W::new(self, 1)
    }
    #[doc = "Bit 2 - G0P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g0p2(&mut self) -> G0p2W<PhmSpec> {
        G0p2W::new(self, 2)
    }
    #[doc = "Bit 3 - G0P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g0p3(&mut self) -> G0p3W<PhmSpec> {
        G0p3W::new(self, 3)
    }
    #[doc = "Bit 4 - G1P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g1p0(&mut self) -> G1p0W<PhmSpec> {
        G1p0W::new(self, 4)
    }
    #[doc = "Bit 5 - G1P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g1p1(&mut self) -> G1p1W<PhmSpec> {
        G1p1W::new(self, 5)
    }
    #[doc = "Bit 6 - G1P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g1p2(&mut self) -> G1p2W<PhmSpec> {
        G1p2W::new(self, 6)
    }
    #[doc = "Bit 7 - G1P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g1p3(&mut self) -> G1p3W<PhmSpec> {
        G1p3W::new(self, 7)
    }
    #[doc = "Bit 8 - G2P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g2p0(&mut self) -> G2p0W<PhmSpec> {
        G2p0W::new(self, 8)
    }
    #[doc = "Bit 9 - G2P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g2p1(&mut self) -> G2p1W<PhmSpec> {
        G2p1W::new(self, 9)
    }
    #[doc = "Bit 10 - G2P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g2p2(&mut self) -> G2p2W<PhmSpec> {
        G2p2W::new(self, 10)
    }
    #[doc = "Bit 11 - G2P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g2p3(&mut self) -> G2p3W<PhmSpec> {
        G2p3W::new(self, 11)
    }
    #[doc = "Bit 12 - G3P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g3p0(&mut self) -> G3p0W<PhmSpec> {
        G3p0W::new(self, 12)
    }
    #[doc = "Bit 13 - G3P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g3p1(&mut self) -> G3p1W<PhmSpec> {
        G3p1W::new(self, 13)
    }
    #[doc = "Bit 14 - G3P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g3p2(&mut self) -> G3p2W<PhmSpec> {
        G3p2W::new(self, 14)
    }
    #[doc = "Bit 15 - G3P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g3p3(&mut self) -> G3p3W<PhmSpec> {
        G3p3W::new(self, 15)
    }
    #[doc = "Bit 16 - G4P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g4p0(&mut self) -> G4p0W<PhmSpec> {
        G4p0W::new(self, 16)
    }
    #[doc = "Bit 17 - G4P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g4p1(&mut self) -> G4p1W<PhmSpec> {
        G4p1W::new(self, 17)
    }
    #[doc = "Bit 18 - G4P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g4p2(&mut self) -> G4p2W<PhmSpec> {
        G4p2W::new(self, 18)
    }
    #[doc = "Bit 19 - G4P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g4p3(&mut self) -> G4p3W<PhmSpec> {
        G4p3W::new(self, 19)
    }
    #[doc = "Bit 20 - G5P0 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g5p0(&mut self) -> G5p0W<PhmSpec> {
        G5p0W::new(self, 20)
    }
    #[doc = "Bit 21 - G5P1 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g5p1(&mut self) -> G5p1W<PhmSpec> {
        G5p1W::new(self, 21)
    }
    #[doc = "Bit 22 - G5P2 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g5p2(&mut self) -> G5p2W<PhmSpec> {
        G5p2W::new(self, 22)
    }
    #[doc = "Bit 23 - G5P3 Schmitt trigger hysteresis mode"]
    #[inline(always)]
    #[must_use]
    pub fn g5p3(&mut self) -> G5p3W<PhmSpec> {
        G5p3W::new(self, 23)
    }
}
#[doc = "Pin hysteresis mode register\n\nYou can [`read`](crate::generic::Reg::read) this register and get [`phm::R`](R).  You can [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`write_with_zero`](crate::generic::Reg::write_with_zero) this register using [`phm::W`](W). You can also [`modify`](crate::generic::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct PhmSpec;
impl crate::RegisterSpec for PhmSpec {
    type Ux = u32;
}
#[doc = "`read()` method returns [`phm::R`](R) reader structure"]
impl crate::Readable for PhmSpec {}
#[doc = "`write(|w| ..)` method takes [`phm::W`](W) writer structure"]
impl crate::Writable for PhmSpec {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
#[doc = "`reset()` method sets PHM to value 0"]
impl crate::Resettable for PhmSpec {
    const RESET_VALUE: u32 = 0;
}
